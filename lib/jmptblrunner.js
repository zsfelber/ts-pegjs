    ...
    JumpTableRunner.prototype.run = function (withToken) {
        ...
        var _loop_1 = function () {
            ...
            var pushShift = function (newShifts, stack, action) {
                ...
                // 2 - based
                currentStates = newShifts.map(function (shift) { return (shift.toStateIndex === 1 ? _this.parseTable.startingState : _this.parseTable.allStates[shift.toStateIndex - 2]); });
                token = owner.next();
                i = 0;
            };
            var hasRecursionSucceeded = function (rsh) {
                // 2 - based
                var reqstate = (shift.toStateIndex === 1 ? _this.parseTable.startingState : _this.parseTable.allStates[rsh.toStateIndex - 2]);
                ...
                if (cached.nextPos !== undefined) {
                    if (cached.result === _2.peg$FAILED) {
                        // TODO failures
                        return false;
                    }
                    else {
                    ...
                    var ruleRefTbl = _1.HyperG.parseTables[rr.ruleIndex];
            ...
            for (; i < currentStates.length; i++) {
                ...
                if (newShifts) {
                    if (!rshs)
                        rshs = new analyzer_rt_1.GrammarParsingLeafStateTransitions();
                    reverseSubStack = [];
                    statesAfterReq = [];
                    nscur = 0;
