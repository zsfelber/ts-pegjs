...
(function (Analysis) {
    ...
    function leafStateCommon(parseTable, index) {
        if (!index)
            return null;
        var ls = Analysis.leafStateCommons[index - 1];
        if (!ls) {
            Analysis.leafStateCommons[index - 1] = ls = new analyzer_rt_1.GrammarParsingLeafStateCommon();
            ls.index = index;
        }
    ...
    function readAllSerializedTables(buf) {
        var pos = 0;
        var _a = [buf[pos++], buf[pos++], buf[pos++], buf[pos++]], stransln = _a[0], sredsln = _a[1], scmnln = _a[2], ctks = _a[3];
