...
(function (Analysis) {
    ...
    function readAllSerializedTables(buf, pos) {
        var stransln = buf[0], sredsln = buf[1];
...
var ParseTable = /** @class */ (function () {
    ...
    ParseTable.prototype.pack = function () {
        ...
        this.allStates.forEach(function (state) {
            ...
            var encoded = index_1.CodeTblToHex(trans.alreadySerialized).join("");
            ...
            function red(rr) {
                ...
                var encred = index_1.CodeTblToHex(rr.alreadySerialized).join("");
...
var GrammarParsingLeafStateTransitions = /** @class */ (function () {
    ...
    GrammarParsingLeafStateTransitions.prototype.ser = function (buf) {
        ...
        if (this.alreadySerialized) {
            this.alreadySerialized.forEach(function (itm) { return buf.push(itm); });
            return;
        }
        var toTknIds = [];
        ...
        es.forEach(function (_a) {
            ...
            if (shifts.length !== 1) {
                ...
                shifts.forEach(function (shift) {
                    toTknIds[posA++] = _this.delta(shift.toState.index);
                    toTknIds[posA++] = shift.shiftIndex;
                });
            }
            else {
                var shift = shifts[0];
                toTknIds[pos++] = _this.delta(shift.toState.index);
...
var GrammarParsingLeafState = /** @class */ (function () {
            ...
            if (!this._transitions) {
                ...
                this.startStateNode.shiftsAndReduces.forEach(function (nextTerm) {
                    if (_this.recursiveShift) {
                        throw new Error("Recursive shift already found, error : " + _this.recursiveShift + "  while generating state:" + _this.startStateNode + " shiftIndex:" + shiftIndex + " unexpected:" + nextTerm.item);
                    }
                    switch (nextTerm.kind) {
    ...
    GrammarParsingLeafState.prototype.deser = function (index, buf, pos) {
        var isrl = buf[0], sndx = buf[1], trind = buf[2], rdind = buf[3], emprdind = buf[4], rsi = buf[5], shi = buf[6];
        pos = 7;
        this.isRule = isrl === 1;
        this.index = index;
