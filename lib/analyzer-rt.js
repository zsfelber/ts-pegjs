...
var GenerateParseTableStackMainGen = /** @class */ (function () {
    ...
    GenerateParseTableStackMainGen.prototype.generate = function (phase) {
            ...
            case 10:
                if (top) {
                    if (this.unresolvedRecursiveBoxes.length) {
                        ...
                        var childrenAffctd = [];
                        unresolvedRecursiveItems.forEach(function (_a) {
        ...
        if (top && deepStats) {
            ...
            var s0 = sum();
            var s = sum();
            summ_1(s0, this.shifts, null);
            this.children.forEach(function (box) {
                box.common["$phase$"] = phase;
                summ_1(s, box.shifts, box);
            });
            console.log(this.indent + "Phase " + phase + " " + this.rule.rule + " :   mainRule    tokens:" + " shifts:" + s0.vShifts + "  recursive deep shifts:" + s0.vRecs);
            console.log(this.indent + "Phase " + phase + " " + this.rule.rule + " :   child boxes tokens:" + " shifts:" + s.vShifts + "  recursive deep shifts:" + s.vRecs + "  prep.tokens:" + s.vPrep);
        }
...
var GenerateParseTableStackBox = /** @class */ (function () {
    ...
    GenerateParseTableStackBox.prototype.appendChild = function (child, recursiveShift, rr) {
        ...
        child_es.forEach(function (_a) {
            ...
            var tokenId = Number(key);
            childShifts.forEach(function (childShift) {
