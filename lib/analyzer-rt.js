...
var GenerateParseTableStackMainGen = /** @class */ (function () {
    ...
    GenerateParseTableStackMainGen.prototype.generate = function (phase) {
        ...
        if (top && deepStats) {
            var sum = function () {
                return {
                    vShifts: new index_1.IncVariator(),
                    vRecs: new index_1.IncVariator(),
                    vPrep: new index_1.IncVariator()
                };
            };
            var summ_1 = function (sum, shifts, box) {
            ...
            var summ2 = function (sum, parseTable) {
                var stack = {};
                parseTable.allStates.forEach(function (state) {
                    if (state) {
                        ...
                        if (common) {
                            if (!stack[common[index_1.UNIQUE_OBJECT_ID]]) {
                                stack[common[index_1.UNIQUE_OBJECT_ID]] = 1;
                                summ_1(sum, common.transitions, null);
                            }
                        }
                    }
                });
...
var GenerateParseTableStackBox = /** @class */ (function () {
    ...
    GenerateParseTableStackBox.prototype.newShift = function (oldShiftIndex, tokenId, shift) {
        ...
        if (olditm) {
            ...
            os.serStackItms(buf);
            var srold = _1.CodeTblToHex(buf);
            buf = [shift.toStateIndex];
            shift.serStackItms(buf);
            var srnew = _1.CodeTblToHex(buf);
            if (srold !== srnew) {
    ...
    GenerateParseTableStackBox.prototype.appendChild = function (child, recursiveShift, rr) {
        var _this = this;
        var child_es = Object.entries(child.shifts.map);
        child_es.forEach(function (_a) {
            var key = _a[0], childShifts = _a[1];
            var tokenId = Number(key);
            var min = index_1.minimum(childShifts, function (a, b) {
                    ...
                    this.startStateNode.shiftsAndReduces.forEach(function (nextTerm) {
                        switch (nextTerm.kind) {
                            case _1.ShiftReduceKind.SHIFT:
                                ...
                                pushToMap_1(s, s.item.node.tokenId, _this.serialStateMap);
                                shiftIndex++;
                                break;
